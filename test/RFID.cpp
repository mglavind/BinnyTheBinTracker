...
void setup() {
    ...
    // delay(3000);
    // Serial.println("RFID program");
    SPI.begin();             // Initialize SPI bus
    mfrc522.PCD_Init();      // Initialize MFRC522

    pinMode(APPEND_MODE_PIN, INPUT_PULLUP);         // Set append mode pin as input
    pinMode(STANDARD_MODE_PIN, INPUT_PULLUP);       // Set standard mode pin as input
    pinMode(RESET_COUNT_PIN, INPUT_PULLUP);         // Set reset count pin as input
}

void loop() {
    // Check append mode pin
    if (digitalRead(APPEND_MODE_PIN) == LOW) {
    ...
    if (digitalRead(STANDARD_MODE_PIN) == LOW) {
    ...
    if (digitalRead(RESET_COUNT_PIN) == LOW) {
        resetCountFlag = true;
